...
GridMvc = (function ($) {
    ...
    */
    ...
    gridMvc.prototype.setupPopupInitialPosition = function (popup) {
        ...
        if (offsetRight < 0) {
            var info = getInfo();
            info.arrow.css({ left: (info.currentArrowLeft) + "px" });
            drop.css({ right: (info.currentDropLeft) + "px" });
        };
        ram = $(".grid-table").width() + 50 - (dropLeft + dropWidth);
        if(ram<0){
            
            var info = getInfo();
            info.arrow.css({ left: ((ram * (-1)) + 12) + "px" });
            drop.css({ right: ram +  + "px" });
            drop.css({ left: "auto" });
            window.scrollTo($(".grid-table").width() + ram, 0);
